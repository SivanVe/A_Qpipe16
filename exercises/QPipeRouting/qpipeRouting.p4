...
register<bit<32>>(1) median_register;
...
control ingress (inout headers hdr,
    ...
    action set_select(bit<8> direction) {
      bit<32> median_val;
      median_register.read(median_val, 0);

      if (direction == 0) {
        meta.meta.selected = 0;
      }
      else { // direction == 1
        if (median_val != 0 && hdr.pq_hdr.isValid()) {
          if (hdr.pq_hdr.value > median_val) {
                        ...
                        if (meta.meta.to_delete_num != 0) {
                            ...
                            if (meta.meta.item_num < meta.meta.len) { // FIXME: was ==
                                ...
                                if (last_array_item_num < last_array_len) {
                                    ...
                                    // ** go to sample option
                                    bit<32> median_pos = ARRAY_LEN_INTOTAL - last_array_len/2;
                                    bit<32> median_val;
                                    a_register.read(median_val, median_pos);
                                    median_register.write(0, median_val);
